<?php
    ...
    if(isset($_GET['dateInicio'])&&isset($_GET['dateFin'])&&isset($_GET['timeInicio'])&&isset($_GET['timeFin'])) {
        ...
        $sql = "SELECT rss.categoria as categoria, count(*) as contador FROM (SELECT categorias.categoria as categoria FROM (SELECT * FROM ordenes WHERE (".$fechaFiltro." between '".$dateTimeInicio."' and '".$dateTimeFin."') AND terminada=1) as rs, categorias WHERE rs.idCategoria=categorias.id) as rss GROUP BY rss.categoria";
